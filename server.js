...
app.post('/scraper/start', (req, res) => {
    try {
        ...
        let command;
        if (isWindows) {
            // For Windows, create a simple batch command with fewer nested quotes
            command = `cmd.exe /c start cmd.exe /k "ping 127.0.0.1 -n 6 > nul & cd ${__dirname} & npx tsx test.ts"`;
        } else {
